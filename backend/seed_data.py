...
async def seed_data():
    ...
    ambulances_data = [
        {"lat": 40.7414, "lng": -73.9690, "status": AmbulanceStatus.IDLE, "updated_at": datetime.utcnow()},
        {"lat": 40.7519, "lng": -73.9881, "status": AmbulanceStatus.IDLE, "updated_at": datetime.utcnow()},
        {"lat": 40.7450, "lng": -73.9750, "status": AmbulanceStatus.IDLE, "updated_at": datetime.utcnow()},
    ]
    
    # Check if ambulances already exist
    existing_ambulances = await Ambulance.find_all().to_list()
