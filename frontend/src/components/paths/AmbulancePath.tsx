...
export default function AmbulancePath({ ambulance }: AmbulancePathProps) {
  ...
  const { core, glow } = useMemo(
    () => colorFromId(String(ambulance.id)),
    [ambulance.id],
  );
  ...
  const geojson = useMemo(
    ...
    [ambulance.id, coordinates],
  );

  const glowLayer: LayerProps = useMemo(
    () => ({
      id: `ambulance-path-glow-${ambulance.id}`,
      type: "line",
      paint: {
        "line-color": glow,
        "line-width": 8,
  ...
  const coreLayer: LayerProps = useMemo(
    () => ({
      id: `ambulance-path-core-${ambulance.id}`,
      type: "line",
      paint: {
        "line-color": core,
        "line-width": 2.5,
  ...
  return (
    <Source id={`ambulance-path-${ambulance.id}`} type="geojson" data={geojson}>
      <Layer {...glowLayer} />
      <Layer {...coreLayer} />
    </Source>
  );
